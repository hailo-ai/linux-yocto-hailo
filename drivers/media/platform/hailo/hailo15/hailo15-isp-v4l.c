...
static int hailo15_isp_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (!enable) {
		...
		atomic_set(&isp_dev->streaming_started, 0);
...
disable:
		if(path == ISP_MCM_IN){
			...
			hailo15_dma_buffer_done(ctx, sd->grp_id, isp_dev->cur_buf[ISP_MCM_IN]);
...

{
	...
	if (src_pad_idx < 0) {
		pr_warn("%s - invalid source pad index %d\n", __func__, src_pad_idx);
		return -EINVAL;
	}
	...
	isp_dev->input_fmt.pad = 0;
	ret = v4l2_subdev_call(sensor_sd, pad, set_fmt, NULL, &isp_dev->input_fmt);
	if (ret) {
					...
					int path)
{
	...
	if (buf) {
		hailo15_dma_buffer_done(ctx, buf->grp_id, buf);
	} else {
		hailo15_dma_buffer_done(ctx, isp_dev->sd.grp_id, buf);
	}
}
...
void frame_timeout_handler(struct timer_list *t) {
	...
	if (!atomic_read(&isp_dev->frame_received)) {
		...
		if (atomic_cmpxchg(&isp_dev->streaming_started, 1, 0)) {
		...
		if (!atomic_cmpxchg(&isp_dev->streaming_started, 0, 1)) {

			// Send stream stat for all sensors - only subscribed sensors will be processed
			for (pad = HAILO15_ISP_SINK_PAD_MAX; pad < HAILO15_ISP_SOURCE_PAD_MAX; ++pad) {
				if(hailo15_process_isp_pad_stat_sub(isp_dev, pad, 1,
