...
static int imx678_set_test_pattern(struct imx678* imx678, int val)
{
	...
	if (TEST_PATTERN_DISABLED == val)
		...
		ret = imx678_write_reg(imx678, IMX678_TPG_PATSEL_DUOUT, 1, val - 1);
		if (!ret) {
			ret = imx678_write_regs(imx678, imx678_tpg_en_regs, ARRAY_SIZE(imx678_tpg_en_regs));
		}
	}
	return ret;
}
...
static int imx678_set_ctrl(struct v4l2_ctrl* ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		ret = __v4l2_ctrl_modify_range(imx678->exp_ctrl,
			IMX678_EXPOSURE_MIN,
			imx678->vblank +
			imx678->cur_mode->height -
	...
	case V4L2_CID_EXPOSURE:
		...
		dev_dbg(imx678->dev, "Received exp %u analog gain %u",
	...
	struct v4l2_subdev_mbus_code_enum* code)
{
	...
	code->code = supported_modes[imx678->selected_mode_idx].code;
	...
	struct v4l2_subdev_frame_size_enum* fsize)
{
	...
	mutex_lock(&imx678->mutex);
	if (fsize->code != supported_modes[imx678->selected_mode_idx].code) {
		mutex_unlock(&imx678->mutex);
		return -EINVAL;
	}

	fsize->min_width = supported_modes[imx678->selected_mode_idx].width;
	fsize->max_width = fsize->min_width;
	fsize->min_height = supported_modes[imx678->selected_mode_idx].height;
	...
	struct v4l2_subdev_format* fmt)
{
	...
	mode = &supported_modes[imx678->selected_mode_idx];
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*framefmt = fmt->format;
		...
		ret = imx678_update_controls(imx678, mode);
		if (!ret)
	...
	struct v4l2_subdev_state* sd_state)
{
	...
	imx678_fill_pad_format(imx678, &supported_modes[DEFAULT_MODE_IDX], &fmt);
...
static int imx678_start_streaming(struct imx678* imx678)
{
	...
	ret = imx678_write_regs(imx678, reg_list->regs,
	...
	if (ret) {
		dev_err(imx678->dev, "fail to setup handler");
		return ret;
	}

	/* Start streaming */
	ret = imx678_write_reg(imx678, IMX678_REG_MODE_SELECT,
		1, IMX678_MODE_STREAMING);
	if (ret) {
		dev_err(imx678->dev, "fail to start streaming");
		return ret;
	}
	/* Start streaming */
	ret = imx678_write_reg(imx678, 0x3002,
		1, 0);
	if (ret) {
		dev_err(imx678->dev, "fail to start streaming");
		return ret;
...
{
	return imx678_write_reg(imx678, IMX678_REG_MODE_SELECT,
		1, IMX678_MODE_STANDBY);
}
...
static int imx678_find_nearest_frame_interval_mode(struct imx678* imx678,
	struct v4l2_subdev_frame_interval* fi, struct imx678_mode const** mode)
{
	...
	for (i = 0; i < sizeof(supported_modes) / sizeof(struct imx678_mode); ++i) {
...
static int imx678_init_controls(struct imx678* imx678)
{
	...
	imx678->exp_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
		&imx678_ctrl_ops,
		V4L2_CID_EXPOSURE,
		IMX678_EXPOSURE_MIN,
		lpfr - IMX678_EXPOSURE_OFFSET,
		IMX678_EXPOSURE_STEP,
		IMX678_EXPOSURE_DEFAULT);

	imx678->again_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
		&imx678_ctrl_ops,
		V4L2_CID_ANALOGUE_GAIN,
		IMX678_AGAIN_MIN,
		IMX678_AGAIN_MAX,
	...
	imx678->vblank_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
		&imx678_ctrl_ops,
		V4L2_CID_VBLANK,
		mode->vblank_min,
		mode->vblank_max,
	...
	imx678->test_pattern_ctrl = v4l2_ctrl_new_std_menu_items(ctrl_hdlr, &imx678_ctrl_ops,
		V4L2_CID_TEST_PATTERN,
	...
	imx678->link_freq_ctrl = v4l2_ctrl_new_int_menu(ctrl_hdlr,
		&imx678_ctrl_ops,
		V4L2_CID_LINK_FREQ,
	...
	imx678->hblank_ctrl = v4l2_ctrl_new_std(ctrl_hdlr,
		&imx678_ctrl_ops,
		V4L2_CID_HBLANK,
		IMX678_REG_MIN,
		IMX678_REG_MAX,
...
static int imx678_probe(struct i2c_client* client)
{
	...
	dev_info(imx678->dev, "imx678 probe start =-=-=-=-=-=-=-=-=-=-=-=-==-\n");
	...
	imx678->cur_mode = &supported_modes[DEFAULT_MODE_IDX];
