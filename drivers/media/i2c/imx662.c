...
/* 2-lane wiring needs higher per-lane data rate to preserve total CSI
 * bandwidth (Sony AllPixel col 10: 30fps FHD 12-bit 2-lane → 06h;
 * DOL_AllPixel col 22: 30fps DOL2 12-bit 2-lane → 04h). Mode register
 * tables keep 4-lane defaults (07h); the override fires only when DT
 * declares 2 lanes, alongside the LANEMODE patch.
 */
#define IMX662_REG_DATARATE_SEL       0x3015
#define IMX662_DATARATE_SEL_SDR_2LANE 0x06 /* 720 Mbps  */
#define IMX662_DATARATE_SEL_HDR_2LANE 0x04 /* 1188 Mbps */
...
static int imx662_start_streaming(struct imx662 *imx662)
{
	 ...
	 * for 2-lane configs, and bump DATARATE_SEL to preserve total CSI
	 * bandwidth.
	 */
	if (imx662->lanes == IMX662_LANES_2) {
		u8 datarate_2lane = imx662->hdr_enabled
			? IMX662_DATARATE_SEL_HDR_2LANE
			: IMX662_DATARATE_SEL_SDR_2LANE;
		...
		ret = imx662_write_reg(imx662, IMX662_REG_DATARATE_SEL,
				       IMX662_REG_VALUE_08BIT,
				       datarate_2lane);
		if (ret) {
			dev_err(imx662->dev,
				"Failed to set 2-lane DATARATE_SEL=0x%02x: %d\n",
				datarate_2lane, ret);
			return ret;
		}
	}
					...
					struct v4l2_subdev_frame_interval *fi)
{
	...
	if (imx662->streaming) {
		...
		mutex_unlock(&imx662->mutex);
		return (match == imx662->cur_mode) ? 0 : -EBUSY;
	}
